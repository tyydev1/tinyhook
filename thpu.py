...
# A tiny package manager made in Python #
...
parser = argparse.ArgumentParser(
    prog="tinyhook", description="TinyHook Package Utils - A minimal package manager"
)
parser.add_argument("--sandbox", action="store_true", help="Run Sandbox™ code")
parser.add_argument(
    "-v", "--version", action="store_true", help="Show TinyHook version"
)
parser.add_argument("--debug", action="store_true", help="Enable debug mode")
parser.add_argument(
    "--dry-run", action="store_true", help="Simulates action without executing it"
)
parser.add_argument(
    "-q", "--quiet", action="store_true", help="Supresses normal output behavior"
)
...
hook_parser.add_argument(
    "--dry-run", action="store_true", help="Simulate installation without performing it"
)
hook_parser.add_argument(
    "-q", "--quiet", action="store_true", help="Supresses normal output behavior"
)
...
if args.command == "hook":
    ...
    if is_installed(package_name):
        ...
        if args.dry_run:
            ...
            if not args.quiet:
                print(f"Successfully hooked '{package_name}'!")
    ...
    if not is_installed(package_name):
        ...
        print(f"[Dry Run] Would run {package_name}")
    ...
    if not installed_data:
        print("No packages installed.")  # If no packages are installed

    elif args.dry_run:
        print("[Dry Run] Would list packages and version")
    ...
    if not installed_data:
        ...
        print(f"[Dry Run] Would remove {package_name}")

    elif not package_name in installed_data:
        print(f"Package '{package_name}' is not found")
...
class Sandbox:
    def __init__(self, name, should_auto_instance=False):
        ...
        if should_auto_instance:
            self.new_instance("Initial Instance")
    ...
    def run(self):
        ...
        try:
            ...
            print("[Failure] No arguments provided")
